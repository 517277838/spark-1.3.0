...
class FileShuffleBlockManager(conf: SparkConf)
   ...
   * when the writers are closed successfully
    * 给每个map task获取一个ShuffleFileGroup
   */
  def forMapTask(shuffleId: Int, mapId: Int, numBuckets: Int, serializer: Serializer,
    ...
    new ShuffleWriterGroup {
      ...
      /**
        * 这里很关键
        * 根据判断consolidateShuffleFiles是否为true，判断是否开启来consolidate机制(也就是优化后的机制)
        * 如果开启了，那么实际上不会给每个bucket都获取一个独立的文件
      ...
      val writers: Array[BlockObjectWriter] = if (consolidateShuffleFiles) {
        ...
        Array.tabulate[BlockObjectWriter](numBuckets) { bucketId =>
          // 首先，用shuffleId, mapId, bucketId(也可以看成reduceID，因为一个bucketId就会对应一个reduceid)生成一个唯一的ShuffleBlockId
          // 然后调用blockId，来调用ShuffleFileGroup的apply()函数，为bucket获取一个ShuffleFileGroup
          val blockId = ShuffleBlockId(shuffleId, mapId, bucketId)
          /**
            * 然后用BlockManager的getDiskWriter()方法，针对ShuffleFileGroup获取一个Writer
            * 这样的话，我们就清楚来，如果开启来consolidation机制
            * 实际上，对于每一个bucket，都会获取一个针对ShuffleFileGroup的writer,而不是一个独立的ShuffleBlockFile的writer
            * 这样就实现来所谓的，多个shuffleMapTask的输出数据的合并
            */
          blockManager.getDiskWriter(blockId, fileGroup(bucketId), serializer, bufferSize,
            writeMetrics)
        }
      } else {
       // 如果没有开启consolidate机制，也就是普通shuffle操作
        Array.tabulate[BlockObjectWriter](numBuckets) { bucketId =>
          // 同样生成一个ShuffleBlockId
          val blockId = ShuffleBlockId(shuffleId, mapId, bucketId)
          //然后调用BlockManager的diskBlockManager ，获取一个代表要写入的本地磁盘文件的blockFile
          val blockFile = blockManager.diskBlockManager.getFile(blockId)
          // Because of previous failures, the shuffle file may already exist on this machine.
          // If so, remove it.
          //判断blockFile是否存在，如果存在删除
          if (blockFile.exists) {
            if (blockFile.delete()) {
          ...
          //然后调用blockManager的getDiskWriter()方法，针对哪个blockFile生成writer
