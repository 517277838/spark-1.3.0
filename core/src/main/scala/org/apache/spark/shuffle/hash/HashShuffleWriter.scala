...
private[spark] class HashShuffleWriter[K, V](
  ...
  /**
    *
    * @param records
    * 将每个ShuffleMapTask计算出来的新的RDD 的partition数据，写入本地磁盘
    */
  override def write(records: Iterator[_ <: Product2[K, V]]): Unit = {
    /**
      * 首先判断，是否需要在map端本地进行聚合
      * 这里，如果是像reduceByKey这样的操作，它的dep.aggregator.isDefined就是true
      * 包括dep.mapSideCombine也是true
      * 那么就会进行map端的本地聚合
      */
    val iter = if (dep.aggregator.isDefined) {
      if (dep.mapSideCombine) {
        //执行本地聚合，比如这里有  (hello,1),(hello,1),聚合后为(hello,2)
    ...
    /**
      * 如果要本地聚合，那么先本地聚合
      * 然后遍历数据
      * 对每一个数据，调用partition，默认是HashPartitoner生成bucketId
      * 也就是决定了，每一份数据，要写入哪个bucket中
      */
    for (elem <- iter) {
      val bucketId = dep.partitioner.getPartition(elem._1)
      //获取到bucketId之后
      //会调用shuffleBlockManager.forMapTask()方法，来生成 bucketId对应的writer,然后用Writer将数据写入bucket
