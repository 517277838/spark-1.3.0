...
private[spark] abstract class Task[T](val stageId: Int, var partitionId: Int) extends Serializable {
  ...
  final def run(taskAttemptId: Long, attemptNumber: Int): T = {
    ...
    //里面纪录了task执行的一些全局性的数据
    //比如说，task重试了几次，包括，task属于那个stage,task要处理的是rdd那个partition等等
  ...
  /**
    ...
    *  而关键的操作，全部都要依赖于子类的实现
    *  Task的子类，有哪些
    *  ShuffleMapTask 、ResultTask 要运行它们的runTask()方法，才能执行我们自己定义的算子和逻辑(在这之前的操作都是为了这个做准备的)
    */
