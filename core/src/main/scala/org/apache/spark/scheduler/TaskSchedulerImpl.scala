...
/**
 * 1.底层通过操作一个SchedulerBackend，针对不同种类的cluster(standalone,yarn)调度task
 * 2.它也可以通过一个LocalBackend,并且将isLocal参数设置为true,来在本地模式下工作
 * 3.它负责处理以一些通用的逻辑，比如决定多个job的调度顺序，启动推测任务执行
 * 3.客户端首先应该调用他的initialize() 方法和 start()方法，然后通过runTasks()方法提交task sets
 * 
 */
/**
  ...
  extends TaskScheduler with Logging
{
  ...
  override def submitTasks(taskSet: TaskSet) {
    ...
    this.synchronized {
      //给每一个TaskSet,都会创建一个TaskSetMannage
    ...
    /** SparkContext原理剖析的时候，讲过，创建TaskScheduler的时候，一件非常重要的事件，就是为了TaskSchedulerImpl
      * 创建一个SparkDeploySchedulerBackend,这里的backend,指的就是之前创建好的SparkDeploySchedulerBackend
      * 而且这个backend是负责创建AppClient,向Master注册Application
      */
  ...
  def resourceOffers(offers: Seq[WorkerOffer]): Seq[Seq[TaskDescription]] = synchronized {
    ...
    //首先，将可用的executor进行shuffle，也就是说，进行打散，从而做到，尽量可以将负责均衡
    val shuffledOffers = Random.shuffle(offers)
    // Build a list of tasks to assign to each worker.
    /**
      * 然后针对WorkerOffer,创建一堆需要用的东西
      * 比如，他tasks，很重要，它是一个二维数组，ArrayBuffer,元素又是一个ArrayBuffer
      * 并且每个ArrayBuffer的数量是固定的，也就是这个executor可用的cpu数量
      */
    ...
    /**
      ...
      * 执行一个initalize()方法，在这个方法中，其实就会创建一个调度池
      * 这里，相当于是说，所有提交的TaskSet,首先呢，会放入这个调度池
      * 然后在执行task分配算法的时候，会从这个调度池，取出排好队的TaskSet
      */
