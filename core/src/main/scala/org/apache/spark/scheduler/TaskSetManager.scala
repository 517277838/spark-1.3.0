 ...
 *                        task set will be aborted
  ...
  * 会负责重试task,直到超过重试的次数限制，并且会通过延迟调度，为这个TaskSet处理本地化调度机制，它的主要接口是
  * resourceOffer,在这个接口中，TaskSet会希望在一个节点上运行一个任务，并且介绍任务的状态改变消息，来直到它负责的task的状态改变了
 */
private[spark] class TaskSetManager(
  ...
  /**
    ...
    * 如果说，本地化级别的等待时间在一定范围内，那么就认为这饿task使用本地化级别可以在executor上启动
    *
    */
  def resourceOffer(
      execId: String,
