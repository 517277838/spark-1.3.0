  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  class DriverActor(sparkProperties: Seq[(String, String)]) extends Actor with ActorLogReceive {
    ...
    def makeOffers() {
      /**
        * 第一步，调用TaskSchedulerImpl的resoutceOffers()方法，执行任务分配算法，将各个task分配到executor上去
        * 第二部，分配好的task到executor之后，执行自己的luanchTasks()方法，将分配的task发送LaunchTask消息
        * 到对应的executor上去，由executor启动并执行task
        *
        * 给resourceOffers方法传人的是什么？
        * 传人的是这个Application所有可用的executor,并且将其封装成了workerOffer，每个WorkerOffer代表了
