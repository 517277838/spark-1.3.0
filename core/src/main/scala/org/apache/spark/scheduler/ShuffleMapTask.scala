...
/**
  ...
  * 基于一个在ShuffleDependency中指定的partition,默认，就是HashPartitioner
  */
/**
...
private[spark] class ShuffleMapTask(
  ...
  override def runTask(context: TaskContext): MapStatus = {
    ...
    /**
      ...
      * 这个rdd，关键问题是，怎么去拿的的？
      * 因为，多个task运行在多个executro中，都是并行运行，或者并发运行的
      * 可能都不在一个地方
      * 但是呢，一个stage的task，其实要处理的rdd是一样的
      * 所以，task怎么拿到自己要处理的那个rdd的数据呢
      * 这里呢，会通过broadcast variable (广播变量)直接拿到
      *
      */
    ...
    try {
      ...
      /**
        ...
        * 所以，核心的逻辑，就是在rdd的iterator()方法中，在这里，就实现了针对rdd的某个partition,执行我们自己定义的算子，或者逻辑
        * 执行完了我们定义的算子，或者函数，会有返回的数据
