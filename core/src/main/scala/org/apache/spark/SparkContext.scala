...
import org.apache.hadoop.io.{ArrayWritable, BooleanWritable, BytesWritable, DoubleWritable, FloatWritable, IntWritable, LongWritable, NullWritable, Writable}
...
/**
  * SparkContext 是spark函数功能主要的入口，扮演里连接spark集群的角色。可以再集群里，创建RDD,累加器，广播变量。
 * Main entry point for Spark functionality. A SparkContext represents(表示) the connection to a Spark
 * cluster, and can be used to create RDDs, accumulators and broadcast variables on that cluster.
 *
  * 只能允许一个sparkcontext存在，再启动一个新的spark任务之前，必须停止前面运行的那个
...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  // 确保sparkContext实列的唯一性

  SparkContext.markPartiallyConstructed(this, allowMultipleContexts)

  // This is used only by YARN for now, but should be relevant(相关的) to other cluster types (Mesos,
  ...
  // An asynchronous listener bus for Spark events(创建监听器)
  ...
  /**
    ...
    * @param minPartitions
    * @return
    *首先，hadoopFile()方法的调用，会创建一个HadoopRDD，其中的元素，其实时(key,value) pair
    * key时HDFS或文本文件的每一行的offset,value就是每一行的字符串值
    * 然后对HadoopRDD调用map()方法，会剔除key,只保留value，然后会获得一个MapPartitionsRDD
    * MapPartitionsRDD内部的元素，其实就是一行一行文本行
    *
    */
  ...
  def hadoopFile[K, V](
    ...
    //将Configuration封装为SerializableWritable用于序列化读写操作，然后广播Hadoop的Configuration
    //Hadoop的Configuration通常只有10K，所有不会对性能有影响
  ...
  private def createTaskScheduler(
    ...
    master match {
      ...
      case LOCAL_N_FAILURES_REGEX(threads, maxFailures) =>
        ...
        scheduler.initialize(backend)
        (backend, scheduler)
          //2.step常用的spark提交模式中的standalone
...
object WritableFactory {
  ...
  //
  String test = "{\"areaIds\":\"110000\",\"phoneOs\":\"1000\",\"pdbs\":\"\",\"sites\":\"\",\"ad_pdb_adction\":\"1\",\"ad_site_adction\":\"0\",\"data_validity\":\"101\"}";
