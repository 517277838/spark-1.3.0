...
/**
  *
  * worker中为application启动的executor，实际上就是启动类这个CoarseGrainedExecutorBackend进程
  */
private[spark] class CoarseGrainedExecutorBackend(
  ...
  override def preStart() {
    ...
    //向driver反向注册发送RegisterExecutor消息
 ...
 //driver注册executor成功之后，会发送回来RegisteredExecutor消息
  //此时CoarseGrainedExecutorBackend，会创建executor对象 作为执行句炳
  //其实它的大部分操作都是通过executro实现的
  override def receiveWithLogging = {
    ...
    case LaunchTask(data) =>
      if (executor == null) {
        ...
        //用内部的执行句炳，Executro，的lauchTask()方法，来启动一个task
  ...
  //发送statusUpdate消息,给SparkDeploySchedulerBackend
