  ...
  extends Logging
{
  ...
  def launchTask(
    ...
    //threadPool，executor内部有一个java 线程池
    //然后呢，这里其实将task封装在一个线程池中（TaskRunner）
    //直接将线程丢入线程池，进行执行
    //线程池是自动实现例排队机智的，也就是说，如果线程池内的线程暂时没有空闲
    //那么丢进来的线程都要排队的
  ...
  class TaskRunner(
    ...
    override def run() {
      ...
      try {
        ...
        //通过网络通信，将需要的文件和jar拷贝过来
        updateDependencies(taskFiles, taskJars)
        //最后，通过正式的反序列化操作，将整个task的数据集反序列化回来
        //用的java的类加载器的原因
        //因为java类加载器有很多功能，比如用反射的方式来动态加载一个类，然后创建这个类的对象
        //还要，比如可以用对指定上下文的相关资源，进行加载和读取
        ...
        /**
          * 关键点，task的run方法，进入该方法
          * 这里的value，对于ShuffleMapTask来说，其实就是MapStatus
          * 封装了ShuffleMapTas计算的数据，输出位置
          * 后面，如果还有一个ShuffleMapTas呢？
          * 那么就会去联系MapOutputTracker,来获取上一个ShuffleMapTask的输出位置，然后通过网络拉去数据
          * ResultTask也是一样
          */
        ...
        /**
          ...
          * 包括运行了多长实际，反序列化耗费了多长实际，JAVA虚拟机GC耗费了多长实际，结果的序列化耗费了多长时间
          * 这些信息都会在sparkUI上显示
          */
        for (m <- task.metrics) {
        ...
        /**
          * 核心方法
          * 其实就是调用了executor所在的GoraseGrainedExecutorBackend的statusUpdate()方法
          */
  ...
  private def updateDependencies(newFiles: HashMap[String, Long], newJars: HashMap[String, Long]) {
    ...
    //这里使用synchronized进行多线程并发访问的同步
    //这样做的原因是因为task实际上是以java线程的方式，在一个CoarseGraineExecutorBackend进程内并发运行的
    //如果在执行业务逻辑的时候，要访问一些共享的资源
    //那么就可能会出现多线程并发访问安全问题
    //所以在这里spark选择进行来多线程并发访问的同步(synchronized)
    //进行多线程并发访问的原因?
    //因为，这里访问来像currentFiles等，这类共享的资源
    synchronized {
